...
try{
 ...
 await page.getByRole('button',{name:'Guardar movimiento',exact:true}).click();await page.waitForFunction(()=>!document.getElementById('editor').open);await page.reload();await page.getByRole('heading',{name:'Tus gastos, en claro.'}).waitFor();assert.match(await page.locator('main').innerText(),/1 registros/);
