...
try{
 ...
 await page.getByRole('button',{name:'Guardar movimiento',exact:true}).click();await page.waitForFunction(()=>!document.getElementById('editor').open);await page.reload();await page.getByRole('heading',{name:'Resumen'}).waitFor();assert.match(await page.locator('main').innerText(),/1 movimientos/);
