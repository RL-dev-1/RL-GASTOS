...
function applyTheme() { document.documentElement.dataset.theme = state.settings.theme; document.body.classList.toggle('privacy', state.settings.privacy); $('privacy').setAttribute('aria-label', state.settings.privacy ? 'Mostrar montos' : 'Ocultar montos en pantalla'); }
...
const title = (name, subtitle) => `<div class="lead"><div class="eyebrow">RL GASTOS</div><h1>${name}</h1><p class="muted">${subtitle}</p></div>`;
function monthNav() { return `<div class="month-nav"><button data-action="month" data-delta="-1" aria-label="Mes anterior">←</button><label class="sr-only" for="month" hidden>Mes seleccionado</label><input id="month" aria-label="Mes seleccionado" type="month" value="${selectedMonth}"><button data-action="month" data-delta="1" aria-label="Mes siguiente">→</button></div>`; }
function rows(entries, { trash = false } = {}) {
  if (!entries.length) return '<div class="empty"><div class="empty-symbol">○</div><strong>Todo empieza con un registro</strong>No hay movimientos con estos filtros.</div>';
  return entries.map(e => `<button class="movement" data-action="${trash ? 'restore' : 'edit'}" data-id="${h(e.id)}"><span class="movement-icon">${trash ? '↶' : e.type === 'income' ? '↙' : '↗'}</span><span class="movement-main"><span class="movement-title">${h(e.note || catName(e.categoryId))}</span><span class="movement-meta">${h(catName(e.categoryId))}${e.subcategory ? ' · ' + h(MICA_SUBCATS.find(s => s.id === e.subcategory)?.label || e.subcategory) : ''} · ${h(payName(e.paymentMethodId) || 'Ingreso')} · ${entryDay(e).slice(8)}${trash ? ' · Restaurar' : ''}</span></span><span class="movement-amount ${e.type === 'income' ? 'income' : ''}">${e.type === 'income' ? '+' : ''}${amount(e.amount)}</span></button>`).join('');
}
...
function renderHome() {
  ...
  let comparison = 'Marcá el mes como revisado cuando hayas completado sus movimientos.';
  ...
  return title('Tus gastos, en claro.', 'Un registro a la vez. Un mes que podés entender.') + monthNav() +
    (draft ? '<button class="button wide" data-action="resume">Continuar borrador pendiente →</button><br><br>' : '') +
    `<div class="grid"><div><section class="hero"><div class="row"><span class="eyebrow">GASTADO EN ${h(monthLabel(selectedMonth))}</span><span class="pill">${total.count} registros</span></div><div class="amount private">${h(money(total.expenses))}</div><div class="hero-footer"><div><span class="muted">Presupuesto restante</span><strong class="${remaining !== null && remaining < 0 ? 'danger' : ''}">${remaining === null ? 'Sin definir' : amount(remaining)}</strong></div><div><span class="muted">${limit === null ? 'Tu presupuesto' : 'Límite del mes'}</span><strong>${limit === null ? '<button class="link-button" data-action="nav" data-tab="budgets">Definir →</button>' : amount(limit)}</strong></div></div></section><div class="status-line"><span class="status-dot"></span><span>Guardado en este dispositivo</span><span aria-hidden="true">·</span><span>${reviewed ? 'Mes revisado' : 'Mes por revisar'}</span></div><div class="hint">${h(comparison)}</div>${total.income ? `<div class="card row"><span>Ingresos registrados</span><strong>${amount(total.income)}</strong></div>` : ''}${favoriteRows.length ? '<div class="section-head"><h2>Tus favoritos</h2></div><div class="favorite-strip">' + favoriteRows.map(e => `<button class="button" data-action="repeat" data-id="${h(e.id)}">${h(e.note || catName(e.categoryId))}</button>`).join('') + '</div>' : ''}<div class="section-head"><h2>En qué se fue</h2><button class="link-button" data-action="nav" data-tab="budgets">Presupuestos →</button></div><div class="card">${categories.length ? categories.map(([id, spent]) => `<button class="category" data-action="category" data-id="${h(id)}"><div class="row"><span>${h(catName(id))}</span><strong>${amount(spent)}</strong></div><div class="bar"><span style="width:${Math.round(spent / total.expenses * 100)}%"></span></div><div class="row small muted"><span>${Math.round(spent / total.expenses * 100)}% del gasto</span><span>${budget?.[id] ? 'Límite ' + amount(budget[id]) : 'Sin límite'}</span></div></button>`).join('') : '<p class="muted">Tus categorías aparecerán al registrar gastos.</p>'}</div></div><div><div class="section-head"><h2>Últimos movimientos</h2><button class="link-button" data-action="nav" data-tab="history">Ver todos →</button></div><div class="card">${rows(sorted(entries).slice(0,6))}</div><div class="hint">${state.settings.lastBackupGenerated ? 'Último backup generado: ' + h(dayKey(state.settings.lastBackupGenerated)) + '. Comprobá que lo guardaste en Archivos.' : 'Protegé tu historial: guardá un backup en Archivos o iCloud.'}<br><button class="link-button" data-action="backup">Generar backup →</button></div></div></div>`;
}
...
function openEditor(existing = null, repeat = false) {
  ...
  renderEditor(); if (!$('editor').open) $('editor').showModal();
  persistDraft();
}
...
async function submitMovements() {
  ...
  try {
    ...
    if (await commit(candidate,'Guardado en este dispositivo.',{ clearDraft:draftKey })) { draft = null; $('editor').close(); render(); }
...
function showSettings() { $('settings-content').innerHTML=settingsHTML(); if (!$('settings').open) $('settings').showModal(); }
async function prepareImport(file) {
  try {
    ...
    $('import-content').innerHTML=`<div class="dialog-top"><h2 id="import-title">Revisar restauración</h2><button class="icon-button" data-action="close-import" aria-label="Cancelar importación">×</button></div><p>Archivo: ${h(file.name)}</p><div class="card"><p><strong>${total.count} movimientos activos</strong></p><p>Gastos ${amount(total.expenses)}</p><p>Ingresos ${amount(total.income)}</p><p>${importCandidate.entries.filter(e=>e.deletedAt).length} en papelera</p></div><div class="hint">Reemplazará los ${existing.count} movimientos actuales. Antes se guardará una copia interna del estado actual, dentro de la misma transacción. No fusiona historiales.</div><div id="import-error" class="form-error" role="alert"></div><button class="button primary wide" data-write data-action="confirm-import">Restaurar este backup</button>`;
...
document.addEventListener('click',async e=>{
  ...
  try {
    ...
    if(action==='close-editor'){captureEditor();await persistDraft();$('editor').close();render();}
    ...
    if(action==='draft-load'){const list=await store.drafts();draft=list[Number(button.dataset.index)];draftKey=draft.draftId||draftKey;$('settings').close();openEditor();}
    ...
    if(action==='print'){const entries=activeEntries(state).filter(e=>inPeriod(e,{month:selectedMonth}));const t=totals(entries);$('main').innerHTML=title('Extracto mensual',h(monthLabel(selectedMonth)))+`<p>${t.count} movimientos · Gastos ${amount(t.expenses)} · Ingresos ${amount(t.income)}</p><div class="card">${rows(sorted(entries))}</div>`;window.print();render();}
...
$('editor').addEventListener('cancel',()=>{captureEditor();persistDraft();});
...
async function start() {
  try {
    ...
    if(activeEntries(state).length && (!last || Date.now()-Date.parse(last)>7*86400000))notice('Tu historial merece un backup externo. Podés generarlo en Exportar.');
