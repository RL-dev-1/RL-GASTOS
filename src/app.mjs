...
function renderHome() {
  ...
  return title('Resumen') + monthNav() +
    ...
    `<div class="grid"><div><section class="hero"><div class="row"><span class="eyebrow">Gastos del mes</span><span class="pill">${total.count} movimientos</span></div><div class="amount private">${h(money(total.expenses))}</div><div class="hero-footer"><div><span class="muted">Presupuesto restante</span><strong class="${remaining !== null && remaining < 0 ? 'danger' : ''}">${remaining === null ? 'Sin definir' : amount(remaining)}</strong></div><div><span class="muted">${limit === null ? 'Presupuesto' : 'Límite del mes'}</span><strong>${limit === null ? '<button class="link-button" data-action="nav" data-tab="budgets">Definir →</button>' : amount(limit)}</strong></div></div></section><div class="status-line"><span class="status-dot"></span><span>Guardado en este dispositivo</span><span aria-hidden="true">·</span><span>${reviewed ? 'Mes revisado' : 'Mes por revisar'}</span></div>${comparison ? `<div class="hint private">${h(comparison)}</div>` : ''}${total.income ? `<div class="card row"><span>Ingresos registrados</span><strong>${amount(total.income)}</strong></div>` : ''}${favoriteRows.length ? '<div class="section-head"><h2>Favoritos</h2></div><div class="favorite-strip">' + favoriteRows.map(e => `<button class="button" data-action="repeat" data-id="${h(e.id)}">${h(e.note || catName(e.categoryId))}</button>`).join('') + '</div>' : ''}<div class="section-head"><h2>Gastos por categoría</h2><button class="link-button" data-action="nav" data-tab="budgets">Presupuestos →</button></div><div class="card">${categories.length ? categories.map(([id, spent]) => `<button class="category" data-action="category" data-id="${h(id)}"><div class="row"><span>${h(catName(id))}</span><strong>${amount(spent)}</strong></div><div class="bar"><span style="width:${Math.round(spent / total.expenses * 100)}%"></span></div><div class="row small muted"><span>${Math.round(spent / total.expenses * 100)}% del gasto</span><span>${budget?.[id] ? 'Límite ' + amount(budget[id]) : 'Sin límite'}</span></div></button>`).join('') : '<p class="muted">Sin gastos en este mes.</p>'}</div></div><div><div class="section-head"><h2>Últimos movimientos</h2><button class="link-button" data-action="nav" data-tab="history">Ver todos →</button></div><div class="card">${rows(sorted(entries).slice(0,6))}</div><div class="hint">${state.settings.lastBackupGenerated ? 'Último backup generado: ' + h(dayKey(state.settings.lastBackupGenerated)) + '. Comprobá que lo guardaste en Archivos.' : 'Sin backup externo generado.'}<br><button class="link-button" data-action="backup">Generar backup →</button></div></div></div>`;
}
...
function navigate(next) { tab = next; historyLimit = 60; render(); $('main').scrollTo({ top:0 }); if (!matchMedia('(prefers-reduced-motion: reduce)').matches) $('main').animate([{opacity:.4},{opacity:1}],{duration:220,easing:'ease-in-out'}); }
